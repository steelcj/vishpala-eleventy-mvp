...
> **Correction, added later**: the File Collections item below claimed
> the migration was "already implemented and verified", checked directly
...
> "CMS structure: File Collections" section). The registry item had
> the same problem, `npm run registry` didn't even exist in
> `package.json` at the time, which is the exact bug a real person hit
> running this document's own advice. The content-gaps and deploy-pipeline
> items below weren't re-checked as part of this correction; verify
...
## Recommended: adopt the File Collections model as the standing convention
...
## Content gaps flagged by `validate.js`, not yet closed
...
## Deploy pipeline, still unconfirmed
...
1. Git repo init + registry regeneration + backend commit-path check ("Before editors touch the CMS at all"), blocks everything else.
2. Confirm deploy target, wire `build:strict` in ("Deploy pipeline, still unconfirmed"), cheap, prevents a real class of silent failure.
3. Get the French accessibility translation done ("Content gaps flagged by `validate.js`, not yet closed"), closes a known, visible gap.
4. Everything in "Deferred, not forgotten", revisit only when it's actually needed, not preemptively.
